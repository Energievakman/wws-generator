...
def absolute_download_url(download_id):
    return BASE_URL.rstrip("/") + f"/download/{download_id}"
...
def get_softr_record(record_id):
    ...
    response = requests.get(url, headers={"Softr-Api-Key": SOFTR_API_KEY}, timeout=30)
    response.raise_for_status()
...
def update_softr_record_with_pdf_url(record_id, file_url):
    if not SOFTR_API_KEY or not SOFTR_DATABASE_ID or not SOFTR_TABLE_ID:
        return False, "Softr API gegevens ontbreken in Render."
    if not SOFTR_WWS_OUTPUT_FIELD:
        return False, "SOFTR_WWS_OUTPUT_FIELD ontbreekt in Render. Download is wel beschikbaar."
    ...
    last_errors = []
    for patch_url in [base_url, base_url + "?fieldNames=true"]:
        for value in file_objects:
            try:
                r = requests.patch(patch_url, headers=headers, json={"fields": {SOFTR_WWS_OUTPUT_FIELD: value}}, timeout=30)
                if r.status_code in (200, 201): return True, "WWS rapport is gekoppeld aan het Softr-record."
                last_errors.append(f"{r.status_code}: {r.text[:350]}")
            except Exception as e:
                last_errors.append(str(e))
    return False, "PDF is gemaakt, maar automatisch koppelen aan Softr lukte niet. Laatste fout: " + " | ".join(last_errors[-2:])
...
def extract_wws_data(pdf_bytes):
    ...
    return {
        ...
        "totaal_punten": regex_find(text, r"Totaal aantal punten\s+([\d,.]+)"),
        "maximale_huurprijs": regex_find(text, r"Maximale huurprijs\s+([€\s\d.,]+)"),
        "woning_punten": regex_find(text, r"Woning\s+([\d,.]+)"),
        "binnenruimtes_punten": regex_find(text, r"Binnenruimtes\s+([\d,.]+)"),
        "buitenruimtes_punten": regex_find(text, r"Buitenruimtes\s+([\d,.]+)"),
    }
...
def create_energievakman_cover(data):
    ...
    mx, rx = 42, 430
    c.setFillColor(colors.black); c.setFont("Courier", 30); c.drawString(mx, height-90, "DE ENERGIEVAKMAN")
...
def wws_upload_for_record(record_id):
    ...
    success, msg = update_softr_record_with_pdf_url(record_id, file_url)
