...
def update_softr_record_with_pdf_url(record_id, file_url, data=None):
    """
    Belangrijk:
    - Punten/huurprijs worden eerst apart bijgewerkt.
    - PDF wordt daarna apart geprobeerd.
    - Als PDF-koppeling faalt of time-out geeft, crasht de app niet meer.

    Let op: voor PDF-koppeling naar hetzelfde Render-domein heeft Gunicorn meestal
    minimaal 2 workers of threads nodig, omdat Softr de downloadlink direct probeert op te halen
    terwijl deze POST-request nog bezig is.
    """
    if not SOFTR_API_KEY or not SOFTR_DATABASE_ID or not SOFTR_TABLE_ID:
    ...
    # 1) Eerst punten en huurprijs bijwerken. Dit mag niet afhankelijk zijn van PDF-koppeling.
    extra_fields = {}
    ...
    # 2) Daarna PDF koppelen. Deze stap mag de response niet meer laten crashen.
    if SOFTR_WWS_OUTPUT_FIELD:
        ...
        for patch_url in urls_to_try:
            for file_value in file_values:
                ...
                try:
                    # Kortere timeout: als Softr blijft hangen op file-fetch, crasht Render niet meer.
                    r = requests.patch(
                        patch_url,
                        headers=headers,
                        json=payload,
    ...
    if pdf_ok and fields_ok:
        return True, "WWS rapport verwerkt: " + "; ".join(messages)

    # Geen crash: rapport/download blijft beschikbaar en punten/huurprijs kunnen alsnog goed zijn.
    return False, "WWS rapport gemaakt. " + "; ".join(messages)
