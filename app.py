...
def update_softr_record_with_pdf_url(record_id, file_url, data=None):
    ...
    for patch_url in [base_url, base_url + "?fieldNames=true"]:
        for value in file_objects:
            fields_payload = {SOFTR_WWS_OUTPUT_FIELD: value}

            if SOFTR_WWS_POINTS_FIELD:
                fields_payload[SOFTR_WWS_POINTS_FIELD] = data.get("totaal_punten", "")

            if SOFTR_WWS_RENT_FIELD:
                fields_payload[SOFTR_WWS_RENT_FIELD] = data.get("maximale_huurprijs", "")

            try:
                r = requests.patch(patch_url, headers=headers, json={"fields": fields_payload}, timeout=30)
                if r.status_code in (200, 201):
                    return True, "WWS rapport, puntenaantal en maximale huurprijs zijn gekoppeld aan het Softr-record."
...
def create_energievakman_cover(data):
    ...
    mx, rx = 42, 430
    c.setFillColor(colors.black); c.setFont("Courier-Bold", 30); c.drawString(mx, height-90, "DE ENERGIEVAKMAN")
